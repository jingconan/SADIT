...
gen_desc1 = {
        ...
        'flow_arrival_rate':'0.05' # flow arrival is poisson distribution. Arrival rate
        }

import numpy as np
# from numpy import loadtxt
dis_interval = 3600
st =  np.array(range(168)) * 3600
np.random.seed(0)
shift1 =  {
        # 'val' : '< ./misc/Robust_Data/traffic_val.txt',
        # 'val' : 0.1 * loadtxt(ROOT  + '/misc/Robust_Data/traffic_val.txt'),
        # 'val' : (0.001 * (1 + np.random.rand(len(st), ))) * st,
        'val' : 0.001 * st, # steady shift
        'time' : st,
