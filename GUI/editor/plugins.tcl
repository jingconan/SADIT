...
array set g_plugins_default {
	...
	{"cored.py"}
	{ 127.0.0.1 4038 1 1 0 "emul=cored" -1 }
}
array set g_plugins {
	{"GUI"}
	{ 0 0 1 0 1 "gui=core" -1 }
...
set iconpath "$LIBDIR/icons/tiny"
...
proc popupPluginsCapConfig { wlan parent } {
    global g_plugins LIBDIR g_cap_in_use
	...
	set img$b [image create photo \
		-file "$LIBDIR/icons/tiny/arrow.${b}.gif"]
...
proc pluginsCapConfigRefreshList { wi } {
	...
	# skip CORE daemons
	if { [lsearch -exact "openvz cored.py" $capname] != -1 } { continue }
...
proc popupCapabilityConfig { channel wlan model types values captions bmp possible_values groups } {
    ...
    set lastgn -1
    foreach type $types {
	if { $cfg == "" } { ;# use model-supplied default values
	    set value [lindex $values $n]
	} else { ;# use existing config value
...
proc popupCapabilityConfigApply { wi channel wlan model types groups } {
    ...
    # node doesn't exist, we are changing the node type
    if { [lsearch $node_list $wlan] == -1 } {
...
proc popupSessionConfig { channel sessionids sessionnames sessionfiles nodecounts sessiondates thumbs opaque } {
    ...
    global g_current_session g_session_choice

    set wi .popupSessionConfig
    catch {destroy $wi}
    toplevel $wi
    wm transient $wi . 
    wm title $wi "Sessions"

    frame $wi.top
    label $wi.top.lab -text \
    	"Select an active CORE session or start a new one:"
    pack $wi.top.lab -side left -padx 4 -pady 4
    pack $wi.top -side top -fill x

    set sessn 0
    set g_session_choice 0
    frame $wi.sess -borderwidth 4
    radiobutton $wi.sess.s$sessn -text "start a new session" \
    		-variable g_session_choice -value 0 \
		-command "$wi.btn.connect configure -text \"New\""
    pack $wi.sess.s$sessn -side top -anchor w -padx 4 -pady 4

    set conn_cmd "destroy $wi; sessionConfig connect $wi $channel"

    foreach sid $sessionids {
	incr sessn
 	if {$sid == $g_current_session} {continue}
	frame $wi.sess.s$sessn
	set idx [expr {$sessn - 1}]
	set sname [lindex $sessionnames $idx]
	# title
        radiobutton $wi.sess.s$sessn.btn -text "session $sid: $sname" \
		-variable g_session_choice -value $sid \
		-command "$wi.btn.connect configure -text \"Connect\""
	pack $wi.sess.s$sessn.btn -side top -anchor w
	bind $wi.sess.s$sessn.btn <Double-B1-ButtonRelease> "$conn_cmd"
	# text label and image thumbnail
	set sfile [lindex $sessionfiles $idx]
	set nodecount [lindex $nodecounts $idx]
	set sdate [lindex $sessiondates $idx]
	set thumb [lindex $thumbs $idx]
	set labtxt "      node count: $nodecount"
	set labtxt "$labtxt\n      file: $sfile\n      $sdate"
	frame $wi.sess.s$sessn.fr
	label $wi.sess.s$sessn.fr.lab -text $labtxt -justify left
	if { [file exists $thumb] } {
	    set thumbimg [image create photo -file $thumb]
	    set width [image width $thumbimg]
	    set height [image height $thumbimg]
	    canvas $wi.sess.s$sessn.fr.thumb -background white -relief sunken \
	    	-bd 2 -width $width -height $height
		#-width [expr $width + 2] -height [expr $height + 2]
	    $wi.sess.s$sessn.fr.thumb create image \
	    	[expr $width / 2] [expr $height / 2] -image $thumbimg
	    pack $wi.sess.s$sessn.fr.thumb -side right -padx 4 -pady 4
	}
	pack $wi.sess.s$sessn.fr.lab -side left 
        pack $wi.sess.s$sessn.fr -side top -anchor w
        pack $wi.sess.s$sessn -side top -anchor w -padx 4 -pady 4
    }

    pack $wi.sess -side top -fill x

    # show this dialog on startup

    # Connect / Shutdown / Cancel buttons
    set shut_cmd "destroy $wi; sessionConfig shutdown $wi $channel"
    set cancel_cmd "destroy $wi"
    frame $wi.btn
    button $wi.btn.connect -text "New" -command $conn_cmd 
    button $wi.btn.shutdown -text "Shutdown" -command $shut_cmd
    button $wi.btn.cancel -text "Cancel" -command $cancel_cmd 
    pack $wi.btn.connect $wi.btn.shutdown $wi.btn.cancel \
...
proc pluginConnect { name cmd retry } {
    global g_plugins
    if { $name == "" } {
	puts "pluginConnect error: name not specified!"
	return -1
    }
    if { ![info exists g_plugins($name)] } { 
...
proc pluginRefresh { plugin } {
	...
	sendRegMessage $sock $DEFAULT_GUI_REG
    }
    default {
...
proc pluginCapsInitialize { node config_name } {
	...
	if { [[typemodel $node].layer] == "LINK" } {
		...
		sendConfReplyMessage $sock $node "emane" $types $vals ""
	    }
	}
    } ;# end foreach cap
