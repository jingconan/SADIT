...
proc parseLinkMessage { data len flags } {
    ...
    array set typesizes {	node1num 4 node2num 4 delay 8 bw 8 per 8 \
	...
	if { $v4addr != -1 } {
	    ...
	    setIfcIPv4addr $node1 wireless "$v4net.0/$vals(if2ipv4mask)"
	}
...
proc parseConfMessage { data len flags channel } {
	...
	switch -exact -- "$type" {
	    ...
	    32 {
		incr current $pad
		binary scan $data @${current}I session
    ...
    # check for node existance
    if { [lsearch $node_list $node] == -1 } {
	if { $model != "services" && [lsearch $MACHINE_TYPES $model] < 0 } {
    ...
    # this could be improved, instead of checking for the hard-coded model
    ...
    if { [lsearch $MACHINE_TYPES $model] != -1 } {
	if { $tflags == 0 } {
	    popupNodeProfileConfig $channel n$nodenum $model $types $values \
...
proc parseFileMessage { data len flags channel } {
    ...
    array set tlvnames { 1 num 2 name 3 mode 4 fno 5 type 6 sname \
			16 data 17 cdata }
    array set tlvsizes { num 4 name -1 mode -3 fno 2 type 2 sname -1 \
			data -1 cdata -1 }
    array set defvals {	num -1 name "" mode -1 fno -1 type -1 sname "" \
			data "" cdata "" }
    ...
    # currently, only the service customization is expecting to receive
    # the File Message
    customizeServiceFile $node $vals(name) $vals(data)
}

proc parseEventMessage { data len flags channel } {
    global showAPI eventtypes g_traffic_start_opt execMode
    set prmsg $showAPI
    set current 0
    set nodenum -1
    set eventtype -1
...
proc parseSessionMessage { data len flags channel } {
    global showAPI g_current_session execMode
    set prmsg $showAPI
    set current 0
    set sessionids {}
    set sessionnames {}
...
proc sendNodeAddMessage { channel node } {
	...
	if { [string range $modeltype 0 4] == "emane" } {
	    set type 0xA
	} elseif { [string range $modeldata 0 4] == "range" } {
	    # send range setting for basic on/off WLAN as opaque node data
	    # a new TLV was not defined because the range calculation will soon
	    # move to cored.py as another wireless model
	    set range [getNodeRange $node]
	    set opaque "range=$range"
	}
    }
    ...
    if { [file dirname $icon] == "$LIBDIR/icons/normal" } {
...
proc sendLinkMessage { channel link type } {
	...
	set per [expr { round($per) }]
    }
    set dup [getLinkDup $link]
    if { $dup == "" } { set dup 0 }
    if { $type != "delete" } {
        incr len [expr {12+12+12+4}] ;# delay,bw,per,dup
	if {$prmsg==1 } {
	...
	puts -nonewline $channel [binary format c2sW {0x5 8} 0 $per]
	puts -nonewline $channel [binary format c2S {0x6 2} $dup]
    }
    # TODO: jitter, mer, burst, mburst
...
proc sendConfRequestMessage { channel node model flags netid opaque } {
	...
	global g_current_session
	set session [format "%x" $g_current_session]
	incr len 8
    }
	...
	if { $session > -1 } { puts -nonewline ",session=0x$session" }
    ...
    set msg3 ""
    if { $session != -1 } {
        set msg3 [binary format c2sI {0x20 4} 0 0x$session ]
    }
    # network ID
    set msg4 ""
    if { $netid != -1 } {
...
proc sendConfReplyMessage { channel node model types values opaque } {
	...
	global g_current_session
	set session [format "%x" $g_current_session]
	set sess_data [binary format c2SI {0x20 4} 0 0x$session]
    }
...
proc deployCfgAPI { sock } {
	...
	if [catch { file copy $LIBDIR/icons/normal/thumb-unknown.gif $tf } e] {
	    puts -nonewline "warning: failed to copy "
	    puts "$LIBDIR/icons/normal/thumb-unknown.gif to $tf\n($e)"
	    set tf ""
	}
    }
    set user $CORE_USER
	...
	if { $type == "rj45" && $g_prefs(enable_rj45) != 1 } {
	    puts "warning: skipping RJ45 node $node because RJ45s are disabled!"
	    continue 
	}
	...
	if { $g_prefs(enable_rj45) != 1 && ([nodeType $lnode1] == "rj45" || \
		[nodeType $lnode2] == "rj45") } { continue }
...
proc sendNodeLinkDefinitions { sock } {
    global node_list link_list eventtypes
    sendEventMessage $sock $eventtypes(definition_state) -1 "" "" 0
    foreach node $node_list { sendNodeAddMessage $sock $node }
...
# ask cored.py to load or save an XML file based on the current session
proc xmlFileLoadSave { cmd name } {
    ...
    # inform cored about nodes and links when saving in edit mode
